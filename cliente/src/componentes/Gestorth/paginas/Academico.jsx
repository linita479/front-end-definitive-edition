...
const Academico = () =>{
    ...
    const buscarUsuario = async (nro_doc) => {
        try {
            const token = '3488625e616171a36fb405847f77afb519d4e451';
            const response = await fetch(`http://localhost:8000/personal_simple/?nro_doc=${nro_doc}`,{
                method: "GET",
                headers: {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const token = "3488625e616171a36fb405847f77afb519d4e451";
            const response = await fetch("http://localhost:8000/academicos/", {
            method: "POST",
            headers: {
                Authorization: `Token ${token}`,
            },
    ...
    return(
        <>
        <div>
            <h1 className="titulo-registro">Registrar formacion academica</h1>
